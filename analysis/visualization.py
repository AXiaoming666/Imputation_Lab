...
def plot_forecast_metrics_by_forecast_model(results):
...
def plot_forecast_metrics_by_imputer(results):
    for dataset in results['dataset'].unique():
        for forecast_metric in ['forecast_mse', 'forecast_mae']:
            result = results[results['dataset'] == dataset]
...
def plot_forecast_metrics_by_missing_type(results):
    for dataset in results['dataset'].unique():
        for forecast_metric in ['forecast_mse', 'forecast_mae']:
            result = results[results['dataset'] == dataset]

            grouped_data = result.groupby(['missing_type','missing_rate'])[forecast_metric].agg(['mean','std']).reset_index()
            ...
            # 获取调色板
            palette = sns.color_palette('bright', n_colors=len(grouped_data['missing_type'].unique()))
            
            # 在每个子图上绘制散点图，点的大小与标准差成正比
            g.map_dataframe(sns.scatterplot, x='missing_rate', y='mean', size='std', hue='missing_type', palette='bright', sizes=(20, 200))
            ...
            g.figure.suptitle(f'{forecast_metric} vs missing_type in {dataset}', y=0.95)

            # 调整子图布局，为标题和图例腾出更多空间
            plt.subplots_adjust(top=0.83)

            # 显示图形
            plt.savefig(f'{save_path}by_missing_type/{dataset}_{forecast_metric}_vs_missing_type.png')
            plt.close()  # 关闭当前图形，避免内存泄漏
...
if __name__ == '__main__':
    ...
    os.makedirs(save_path + 'by_missing_type/', exist_ok=True)
    
    results = pd.read_csv('results.csv')
    
    plot_forecast_metrics_by_forecast_model(results)
    
    plot_forecast_metrics_by_imputer(results)
    
    plot_forecast_metrics_by_missing_type(results)